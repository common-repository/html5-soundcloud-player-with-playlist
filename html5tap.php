...
$_sourcetype = get_post_meta($post->ID, '_sourcetype', true);
...
function add_html5tap_metaboxes() {
	...
	add_meta_box('metabox_json', 'JSON API Information', 'metabox_json', 'html5tap', 'normal', 'low');
}
...
function metabox_embed()
{
	...
	$embedFull =  htmlentities(jsWidget('https://player.html5tap.com/v1/html5full.html?jdata='.urlencode(get_the_permalink($post->ID)) ) );
	
	$embedBig =  htmlentities(jsWidget('https://player.html5tap.com/v1/html5big.html?jdata='.urlencode(get_the_permalink($post->ID)), 347, 414) );
...
function metabox_json()
{
  global $post;
  
  $jsonurl = get_the_permalink($post->ID);
  $jsonurl .= (parse_url(get_the_permalink($post->ID), PHP_URL_QUERY) ? '&' : '?') . 'embed=1';
  
  echo "If you have problem getting below URL to work please edit <b>Settings >> Permalink >> Common Settings >> Save Changes</b><br /><br />";
  	 
  echo '<a href="'.$jsonurl.'" target="_blank">'.$jsonurl.'</a>';	 
...
<input name="Submit" type="button" value="Capture"  onClick="playlistSourceType()" /><br /><br />


If you have problem in player playlist to work please edit <b>Settings >> Permalink >> Common Settings >> Save Changes</b><br />
...
function save_html5tap_meta($post_id, $post) {
    ...
    //print_r($html5tap_meta['_title']); die;
...
echo '<div><h1>HTML5Tap Help</h1>';
echo '<a href="http://html5.svnlabs.com/faq/" target="_blank">frequently asked questions</a>';
echo '</div>';

echo '<div>';
echo "If you have problem in player playlist to work please edit **Settings >> Permalink >> Common Settings >> Save Changes**<br /><br />";
echo "Shortcode for Page or Post [html5tap id=1]";
echo '</div>';

echo '<div>&nbsp;</div>';

echo '<div>';
echo "**Important:** before updating HTML5 MP3 Player with Playlist Plugin, please back up your database and files. For help with updates, visit our website, Paid Plugin Users don't upgrade to Free Plugin.<br />";

echo "**Free HTML5 MP3 Player** HTML5Tap players are hosted on CDN https://player.html5tap.com/v1 so for best performance we are using iframe based embed code in this plugin from CDN. <br />";

echo '<a href="http://html5plus.svnlabs.com/shop/html5-mp3-player-for-wordpress/" target="_blank">[Buy Now HTML5 MP3 Player]</a><br />';

echo '</div>';

echo '<div>';
echo '<a href="http://html5.svnlabs.com/contact-us/" target="_blank">Contact US</a>';
echo '</div>';

echo '<div>&nbsp;</div>';

echo '<div>';
echo '<a href="https://twitter.com/HTML5MP3Player" class="twitter-follow-button" data-show-count="false">Follow @HTML5MP3Player</a><script async src="//platform.twitter.com/widgets.js" charset="utf-8"></script>';
echo '</div>';

echo '<div>&nbsp;</div>';

echo '<div>';
echo '<iframe src="https://www.facebook.com/plugins/like.php?href=https%3A%2F%2Fwww.facebook.com%2FHtml5Mp3Player%2F&width=450&layout=standard&action=like&size=small&show_faces=true&share=true&height=80&appId=181968385196620" width="450" height="80" style="border:none;overflow:hidden" scrolling="no" frameborder="0" allowTransparency="true"></iframe>';
echo '</div>';
...
//echo jsWidget('https://player.html5tap.com/v1/html5full.html?jdata='.urlencode(get_the_permalink($atts['id'])) );
return jsWidget('https://player.html5tap.com/v1/html5full.html?jdata='.urlencode(get_the_permalink($atts['id'])) );
